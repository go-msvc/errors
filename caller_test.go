...
func TestCaller(t *testing.T) {
	...
	if c.Package() != "github.com/go-msvc/errors_test" {
		t.Fatalf("Package=%s != github.com/go-msvc/errors_test", c.Package())
	}
	...
	tests := []struct {
		...
		{"%S", fmt.Sprintf("github.com/go-msvc/errors_test/caller_test.go(%d)", lineNr)},
		{"%.5S", fmt.Sprintf("github.com/go-msvc/errors_test/caller_test.go(%5d)", lineNr)},
		{"%f", fmt.Sprintf("TestCaller(%d)", lineNr)},
		{"%.5f", fmt.Sprintf("TestCaller(%5d)", lineNr)},
		{"%F", fmt.Sprintf("github.com/go-msvc/errors_test.TestCaller(%d)", lineNr)},
		{"%.5F", fmt.Sprintf("github.com/go-msvc/errors_test.TestCaller(%5d)", lineNr)},
